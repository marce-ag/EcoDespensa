...
struct VistaEscanearProducto: View {
    ...
    var codigoValido: Bool {
        ...
        let longitud = codigoEscaneado.count
    ...
    func esProductoDeComida(_ producto: OpenFoodFactsProduct) -> Bool {
        ...
        let categoriasTags = producto.categories_tags?.joined(separator: " ").lowercased() ?? ""
        let gruposComida = producto.food_groups_tags?.joined(separator: " ").lowercased() ?? ""

        let textoCompleto = categorias + " " + categoriasTags + " " + gruposComida
        ...
        let palabrasComida = [
            ...
            "dairy", "milk", "yogurt", "cheese", "lacteo", "lacteos", "leche", "queso",
            "meat", "carne", "chicken", "pollo", "fish", "pescado",
            "cereal", "cereals", "snack", "snacks",
            "bread", "pan", "cookies", "galletas",
            "sauce", "salsa", "pasta", "rice", "arroz",
            "chocolate", "candy", "dulce", "dulces"
        ]
    ...
    func buscarProductoPorCodigo(_ codigo: String) async {
        ...
        let urls = [
            ...
            "https://world.openfoodfacts.org/api/v2/product/\(codigo).json?fields=product_name,brands,quantity,categories,categories_tags,food_groups_tags"
        ]

        for urlTexto in urls {
            ...
            do {
                ...
                if respuesta.status == 1,
                   let producto = respuesta.product {

                    await MainActor.run {

                        if let nombreAPI = producto.product_name,
                           !nombreAPI.isEmpty {

                            nombre = nombreAPI
                        }

                        if let cantidadAPI = producto.quantity,
                           !cantidadAPI.isEmpty {

                            separarCantidadDesdeAPI(cantidadAPI)
                        }

                        if let categoriasAPI = producto.categories,
                           !categoriasAPI.isEmpty {

                            categoria = convertirCategoriaAPI(categoriasAPI)
                        }

                        mensajeAPI = "✅ Producto encontrado automáticamente."
